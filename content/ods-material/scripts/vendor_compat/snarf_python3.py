...
Python 3 compatibility copy of the upstream Python 2 script.  The only
compatibility changes are print-function syntax, the interpreter line, and
explicit UTF-8 text I/O.  Conversion rules and emitted strings are unchanged.
"""
import sys
import re
...
def print_code(clazz, methods):
    ...
    try:
        ...
        code = open(filename, encoding='utf-8').read().splitlines()
        printing = False
        for line in code:
            if printing:
...
def fig_subs(line):
    # Python 3 rejects the upstream pattern's unknown ``\i`` escape.
    # Escaping the literal command backslash preserves the intended match.
...
if __name__ == "__main__":
    ...
    if len(argv) == 1:
        ...
        try:
            infile = open(argv[1], encoding='utf-8')
        except IOError:
