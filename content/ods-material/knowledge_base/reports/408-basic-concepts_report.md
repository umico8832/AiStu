# 知识库章节报告：408-basic-concepts

生成时间：2026-07-24T22:16:23.993035Z

## 一、数量统计

- 源记录总数：9（规划强制覆盖的正文承载类 9 条）
- 生成知识点：internal 9 个，published 9 个
- 各类型知识点数量：
  - concept: 4
  - formula: 3
  - theorem: 1
...
- 平均每个知识点引用源记录：1.0 条
- 重复候选处理记录：0 条
- 前置关系（prerequisite 边）：7 条；关系总数（含前置）：16 条
- 未解决关系：0 条
- RAG 检索块：36 个（每知识点 3～4 块）
- 可视化候选：9 条，其中 high 优先级 0 条
...
- `cs408-complexity-estimation`（medium / algorithm_execution）：该知识点包含可逐步演示的结构变化或算法过程，适合进入课件候选评审。

## 四、仍需人工或第二个 AI 审查的内容

- 全部 9 个 `review_pending` 知识点均需外部审查（本流水线不自行判定通过）。
