...
| 工作流 | 状态 | 当前结果 | 下一检查点 |
| --- | --- | --- | --- |
| Electron 与安全边界 | 完成 | 主对话窗口、独立课件窗口、安全隔离、应用协议、权限与导航拦截、Fuses | 最终发布包复验 |
| 对话与 Provider | 完成 | 多会话、流式状态、DeepSeek API、Codex CLI、Demo Provider、结构化命令 | DeepSeek 真实环境 smoke |
...
| 课程学习足迹 | 完成 | 有效时长、学习日期、知识接触、课件互动、轻量成就与错题复盘 | 边界与持久化人工复验 |
...
| 视觉与演示素材 | 完成 | 品牌、最终截图、旁白、转场与演示视频已完成 | 随 Release 发布视频链接 |
| macOS Release | 待完成 | 本地 `.app`、`.dmg`、`.zip` 已生成并启动验证 | GitHub Release |
| 演示视频 | 完成 | 最终成片已完成 | 随 GitHub Release 发布 |
...
- DeepSeek API、Codex CLI 与确定性 Demo Provider；
...
- 商店首页可按考试、简称、类别和旗下科目实时搜索，包含结果计数、清空和无结果
  反馈；
- 只有 408 数据结构标为第一方课程可直接学习；其他目录课程使用独立的添加到课程库
  操作，不复用开始学习路由；
...
| 风险或限制 | 影响 | 当前处理 |
| --- | --- | --- |
| DeepSeek 依赖用户自己的 API Key 与网络 | 未配置或网络不可用时无法使用真实模型 | 提供一处 `.env` 配置与 Demo Provider |
...
| 最终 Release 与视频链接尚未发布 | 黑客松交付尚未闭环 | 作为唯一 P0 工作流推进 |
...
- `pnpm package:dir`
- `pnpm package`
- 打包应用启动、知识快照与本地临时签名完整性检查
- Fuses 在构建期由 `after-pack.cjs` 设置（构建失败则中断）
...
| 2026-07-25 | 最终演示视频完成 | 待随 GitHub Release 发布链接 |
| 待完成 | 最终 GitHub Release | 发布后新增报告 |
