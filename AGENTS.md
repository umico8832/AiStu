...
- 默认不添加副标题、辅助说明、脚注或免责声明。任何新增文字都必须直接帮助用户完成
  当前操作、识别当前状态或处理当前错误；仅用于“解释一下”“显得完整”“避免误解”
  的文字不进入界面；
- 未经用户当前任务明确要求，不得在标题、卡片、面板、表单或页脚附近自行添加解释性
  副文案、安抚性提示、规则复述或免责声明，例如“之后可以随时修改”“不把看过算作
  掌握”“仅反映参与、不代表掌握”等；
- 禁止展示用户无法据此采取行动的内部实现或治理状态，例如 Schema/参数已校验、
  revision、教学已审查、教学模型限制、编译器或平台差异、安全回退机制、本地数据域
  边界等；
- 禁止复述控件已经表达的交互规则，例如按钮已写“打开课件”时，不再补充“只有确认后
  才会打开”；状态标签已表达“待审核”时，不再解释审核流程；
- 禁止添加通用 AI 免责声明、能力边界声明、掌握度免责说明、数据不会如何处理等
  产品自我辩护式文案。确有风险或错误时，只提供当前问题和可执行的恢复操作；
- 标题、控件标签或就近状态已经能表达含义时，不再补充重复说明；删除说明文案后同步
  收紧空白并清理失效的 ARIA 关联；
- 不把 Agent 注释、实现边界、Schema、测试提示、架构说明或维护者备注写进产品正文；
- 不用大段免责声明解释尚未实现的功能；
- 功能不可用时使用简短状态、禁用按钮和就近操作反馈；
- AI 文本只渲染受限 Markdown，不解析模型提供的 HTML；
- 修改 UI 后检查所有小号灰色文字、卡片副标题、表单脚注和页脚，删除其中不能改变
  用户下一步行动的内容，并检查是否残留开发者导向文案。
